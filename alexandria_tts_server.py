...
def generate_batch_clone(texts_json: str, ref_audio, ref_text: str, language: str):
    """fn_index=2 — api_name=/generate_batch_clone — Batch voice clone avec cache prompt"""
    try:
        ...
        print(f"[batch_clone] {len(texts)} textes | ref={os.path.basename(ref_path)}")

        # Detecte le LoRA depuis le ref_path et utilise le cache
        cached_key = None
        for lora_name in _clone_prompt_cache:
            if lora_name in ref_path:
                cached_key = lora_name
        ...
        if cached_key:
            print(f"[batch_clone] Cache hit: {cached_key}")
            prompt = _clone_prompt_cache[cached_key]
        else:
            if ref_path not in _clone_prompt_cache:
                print(f"[batch_clone] Calcul prompt pour {os.path.basename(ref_path)}...")
                audio_arr, sample_rate = sf.read(ref_path)
                if audio_arr.ndim > 1:
                    audio_arr = audio_arr.mean(axis=1)
                prompt = model.create_voice_clone_prompt(
        ...
        # Génère tous les textes en un seul appel GPU
        ...
        # Sauvegarde chaque WAV séparément dans /tmp/batch_wavs/
        ...
        # Retourne le dossier comme JSON pour Alexandria
