...
Imite exactement l'interface que tts.py/_external_generate_custom attend :
  fn_index=0 : generate_custom_voice(text, lang, speaker, instruct) -> audio
  fn_index=1 : generate_clone_voice(ref_audio, ref_text, text, lang) -> audio

Lance avec :
  python3 alexandria_tts_server.py --port 7860
"""
import argparse, os, tempfile, numpy as np
import gradio as gr

# ── Chargement lazy des modèles Qwen3-TTS ───────────────────────
_custom_model = None
_clone_model  = None
...
# Voix disponibles Alexandria -> id interne Qwen3-TTS
VOICE_MAP = {
    "Aiden":    "Aiden",
    "Dylan":    "Dylan",
    "Eric":     "Eric",
...
def generate_custom(text: str, lang: str, speaker: str, instruct: str):
    """
    fn_index=0 — Custom Voice
    Paramètres envoyés par Alexandria/_external_generate_custom :
      text, "Auto", voice_name, instruct_text
    """
    try:
        ...
        return tmp.name, None   # (audio_path, error_message)
    except Exception as e:
        import traceback; traceback.print_exc()
        return None, str(e)


def generate_clone(ref_audio, ref_text: str, text: str, lang: str):
    """
    fn_index=1 — Voice Clone
    Paramètres envoyés par Alexandria/_external_generate_clone :
      handle_file(ref_audio), ref_text, text, "Auto"
    """
    try:
...
def build_app():
    with gr.Blocks() as app:
        ...
        with gr.Tab("Custom Voice"):
            ...
            btn0.click(generate_custom, inputs=[txt0, lang0, spk0, ins0], outputs=[out0, err0])

        # fn_index=1 : Voice Clone
        with gr.Tab("Voice Clone"):
            ref1  = gr.Audio(label="Reference Audio", type="filepath")
            rtxt1 = gr.Textbox(label="Reference Text")
            txt1  = gr.Textbox(label="Text to Synthesize")
            ...
            btn1.click(generate_clone, inputs=[ref1, rtxt1, txt1, lang1], outputs=[out1, err1])
