...
def _get_clone_model():
    ...
    if _clone_model is None:
        ...
        # Precache tous les prompts LoRA au demarrage
        _precache_lora_prompts()
    return _clone_model

def _precache_lora_prompts():
    """Precache les prompts clone pour tous les LoRA disponibles."""
    import soundfile as sf, json
    global _clone_prompt_cache
    if not os.path.exists(LORA_DIR):
        return
    for lora_name in os.listdir(LORA_DIR):
        lora_path = os.path.join(LORA_DIR, lora_name)
        ref_wav = os.path.join(lora_path, "ref_sample.wav")
        meta_path = os.path.join(lora_path, "training_meta.json")
        if not os.path.exists(ref_wav) or not os.path.exists(meta_path):
            continue
        if lora_name in _clone_prompt_cache:
            continue
        try:
            with open(meta_path, "r", encoding="utf-8") as f:
                meta = json.load(f)
            ref_text = meta.get("ref_sample_text", "")
            audio_arr, sample_rate = sf.read(ref_wav)
            if audio_arr.ndim > 1:
                audio_arr = audio_arr.mean(axis=1)
            print(f"[precache] Calcul prompt pour {lora_name}...")
            prompt = _clone_model.create_voice_clone_prompt(
                ref_audio=(audio_arr, sample_rate), ref_text=ref_text)
            _clone_prompt_cache[lora_name] = prompt
            print(f"[precache] {lora_name} mis en cache.")
        except Exception as e:
            print(f"[precache] Erreur pour {lora_name}: {e}")
...
def generate_clone_voice(ref_audio, ref_text: str, text: str, language: str):
    ...
    try:
        ...
        # Detecte si ref_path correspond a un LoRA connu (par nom de dossier dans le path)
        cached_key = None
        for lora_name in _clone_prompt_cache:
            if lora_name in ref_path:
                cached_key = lora_name
                break

        if cached_key:
            print(f"[cache] Reutilisation prompt pour {cached_key}")
            ...
            if ref_path not in _clone_prompt_cache:
                print(f"[cache] Calcul prompt pour {os.path.basename(ref_path)}...")
...
def generate_batch_clone(texts_json: str, lora_name: str, language: str):
    """fn_index=2 — api_name=/generate_batch_clone — Batch via lora_name (pas de upload ref)"""
    try:
        ...
        print(f"[batch_clone] {len(texts)} textes | lora={lora_name}")

        if lora_name not in _clone_prompt_cache:
            lora_path = os.path.join(LORA_DIR, lora_name)
            ref_wav = os.path.join(lora_path, "ref_sample.wav")
            meta_path = os.path.join(lora_path, "training_meta.json")
            if not os.path.exists(ref_wav):
                return None, f"ref_sample.wav not found for {lora_name}"
            with open(meta_path, "r", encoding="utf-8") as f:
                ref_text = json.load(f).get("ref_sample_text", "")
            audio_arr, sample_rate = sf.read(ref_wav)
            if audio_arr.ndim > 1:
                audio_arr = audio_arr.mean(axis=1)
            print(f"[batch_clone] Calcul prompt pour {lora_name}...")
            prompt = model.create_voice_clone_prompt(
                ref_audio=(audio_arr, sample_rate), ref_text=ref_text)
            _clone_prompt_cache[lora_name] = prompt
        else:
            print(f"[batch_clone] Cache hit: {lora_name}")

        prompt = _clone_prompt_cache[lora_name]

        # Vide le cache GPU avant génération pour éviter OOM
        import torch
        ...
        torch.cuda.empty_cache()
        batch_dir = tempfile.mkdtemp(prefix="batch_wavs_")
        wav_paths = []
        ...
        for i, wav in enumerate(wavs_list):
            wav = _trim_start(wav, sr)
            wav = _add_silence(wav, sr)
            out = os.path.join(batch_dir, f"chunk_{i:04d}.wav")
            sf.write(out, wav, sr)
            wav_paths.append(out)
        # Encode les WAVs en base64 pour les transmettre directement
        import base64
        wav_b64 = []
        for wp in wav_paths:
            with open(wp, 'rb') as wf:
                wav_b64.append(base64.b64encode(wf.read()).decode('utf-8'))

        result = json.dumps(wav_b64)
        tmp = tempfile.NamedTemporaryFile(suffix=".json", delete=False, mode='w')
        tmp.write(result)
        tmp.close()
        print(f"[batch_clone] {len(wav_paths)} WAVs encodés en base64")

        # Nettoyage automatique des fichiers tmp Gradio anciens (>1h)
        try:
            import glob as _gl, time as _time
            cutoff = _time.time() - 3600
            for _f in _gl.glob("/tmp/gradio/**/*", recursive=True):
                try:
                    if os.path.isfile(_f) and os.path.getmtime(_f) < cutoff:
                        os.remove(_f)
                except Exception:
                    pass
            for _f in _gl.glob("/tmp/batch_wavs_*"):
                try:
                    import shutil as _sh
                    if os.path.isdir(_f) and os.path.getmtime(_f) < cutoff:
                        _sh.rmtree(_f)
                except Exception:
                    pass
        except Exception:
...
def build_app():
    ...
    with gr.Blocks() as app:
        ...
        with gr.Tab("Batch Clone"):
            ...
            bbtn.click(generate_batch_clone, inputs=[btexts, blora, blang], outputs=[bout, berr], api_name="generate_batch_clone")
