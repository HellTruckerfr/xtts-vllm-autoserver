...
def _get_custom_model():
    ...
    if _custom_model is None:
        ...
        _custom_model = Qwen3TTSModel.from_pretrained("Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice")
...
def _get_clone_model():
    ...
    if _clone_model is None:
        ...
        _clone_model = Qwen3TTSModel.from_pretrained("Qwen/Qwen3-TTS-12Hz-1.7B-Base")
...
def generate_custom_voice(text: str, language: str, speaker: str, instruct: str, model_size: str = "1.7B", seed: int = -1):
    ...
    try:
        ...
        # Ajoute un point final si absent — evite la troncature de la derniere syllabe
        text_padded = text.strip()
        if text_padded and text_padded[-1] not in '.!?\u2026\xbb"\u2019':
            text_padded = text_padded + '.'

        wavs, sr = model.generate_custom_voice(
            text=text_padded, instruct_text=instr, speaker=spk_id,
            language=language if language and language != "Auto" else None,
        )
        wav = np.concatenate(wavs) if isinstance(wavs, list) else wavs
        # Ajoute 300ms de silence a la fin pour eviter la troncature
        silence = np.zeros(int(sr * 0.3), dtype=wav.dtype)
    ...
    with gr.Blocks() as app:
        ...
        with gr.Tab("Custom Voice"):
            ...
            btn0.click(
                ...
                inputs=[txt0, lang0, spk0, ins0, mdl0, sed0],
                outputs=[out0, err0],
