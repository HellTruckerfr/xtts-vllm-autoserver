...
# Telecharge les modeles LoRA depuis HuggingFace
echo ">>> Telechargement des modeles LoRA..."
HF_TOKEN=$(cat /root/.hf_token)
mkdir -p /workspace/lora_models
source /workspace/tts_env/bin/activate
huggingface-cli download Helltrucker/audiobook-lora-models \
    --local-dir /workspace/lora_models \
    --token $HF_TOKEN \
    --repo-type model
deactivate
echo ">>> LoRA modeles telecharges."
