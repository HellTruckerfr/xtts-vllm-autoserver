...
pip install qwen-tts gradio transformers accelerate soundfile

# Télécharge le serveur wrapper compatible Alexandria
curl -s https://raw.githubusercontent.com/HellTruckerfr/xtts-vllm-autoserver/main/alexandria_tts_server.py   -o /workspace/alexandria_tts_server.py
deactivate
...
echo ">>> Creation du template Mistral pour vLLM..."
cat > /workspace/mistral_template.jinja << 'JINJAEOF'
{{ bos_token }}{% for message in messages %}{% if message['role'] == 'system' %}{% if loop.first %}{{ '[INST] ' + message['content'] + '

' }}{% endif %}{% elif message['role'] == 'user' %}{% if not loop.first %}{{ '[INST] ' }}{% endif %}{{ message['content'] + ' [/INST]' }}{% elif message['role'] == 'assistant' %}{{ message['content'] + eos_token }}{% endif %}{% endfor %}
JINJAEOF
...
[program:qwen3-tts]
command=/workspace/tts_env/bin/python3 /workspace/alexandria_tts_server.py --host 0.0.0.0 --port 7860
directory=/workspace
autostart=true
autorestart=true
startsecs=15
