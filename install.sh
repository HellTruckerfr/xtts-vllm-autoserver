...
if [ -z "$HF_TOKEN" ]; then
  echo "WARNING: HF_TOKEN non defini"
else
  echo ">>> Telechargement de Mistral..."
  ...
  for f in config.json generation_config.json special_tokens_map.json tokenizer.json tokenizer.model model.safetensors.index.json model-00001-of-00003.safetensors model-00002-of-00003.safetensors model-00003-of-00003.safetensors; do
    wget -q --header="Authorization: Bearer $HF_TOKEN" "https://huggingface.co/mistralai/Mistral-7B-Instruct-v0.3/resolve/main/$f" -O "$f" && echo "OK $f" || echo "FAIL $f"
  done
  cd /workspace
fi
